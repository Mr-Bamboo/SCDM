...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q.permute(0, 2, 3, 1).view(b, h * w, c).permute(0, 2, 1)
        k = k.view(b, c, h * w).permute(0, 2, 1)
        v = v.permute(0, 2, 3, 1).view(b, h * w, c).permute(0, 2, 1)
...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attention2 = nn.Identity() if not use_attention else AttentionBlock2(out_channels, norm, num_groups)
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.init_rgb = nn.Conv2d(input_channels, base_channels, 3, padding=1)
        ...
        for i, mult in enumerate(channel_mults):
            ...
            for _ in range(num_res_blocks):
                ...
                channels.append(now_channels*2)
                self.ch.append(now_channels * 2)
        ...
        self.out_rgb = nn.Conv2d(base_channels, 3, 3, padding=1)
        #

    def forward(self, x, rgb, time=None, y=None):
        ip = self.initial_pad
        if ip != 0:
            x = F.pad(x, (ip,) * 4)
        ...
        for layer in self.downs:
            rgb = layer(rgb, time_emb, y)
            skips_rgb.append(rgb)
        ...
        for layer in self.ups:
            if isinstance(layer, ResidualBlock):
                ...
                x = torch.cat([x, skips_rgb.pop()], dim=1)
            x = layer(x, time_emb, y)
